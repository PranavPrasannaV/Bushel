...
fire perimeter (WFIGS active, or FRAP historic polygon)
  ...
  │       dispersal: Gill et al. 2022 — 200 m general / 100 m non-serotinous
  │       apply Baker (2023) 21.9% as the default share of high-severity area
  │       ** THIS IS THE DEMO PEAK — the interior lights up **
  │
  ├─4─ PARTITION: 85 seed zones × 500-ft elevation bands ──► cells
  │       species allocated per cell from LANDFIRE pre-fire vegetation
  │       PROVENANCE LOCK: refuses to merge cells
  │
  └─5─ CONVERT per cell, per species, each step citing its source:
          acres  ──► trees            (planting density)
...
Use **Baker (2023)** — seed-limited interior averages 21.9% of high-severity burn area — as the **default**, stated on screen as *"the published estimate least favourable to this conclusion."* One number, not a range. No toggle: nobody can operate a control in a video, and a moving headline number reads as uncertainty.
