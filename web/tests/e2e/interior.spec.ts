...
test('the interior is drawn as the luminous layer, with the threshold statement and a cross-check', async ({
  ...
  // FR-006: nothing on the map can change the threshold or the reference fraction: no control on it at all.
  const region = page.locator('.map-region')
  await expect(region.locator('input, select, textarea, button, [role="slider"]')).toHaveCount(0)
})
